...
Дипломный fullstack-проект: веб-система для создания, публикации, прохождения и анализа образовательных тестов. Приложение поддерживает ролевую модель, конструктор вопросов, автоматическую проверку ответов, сохранение результатов, визуальную аналитику преподавателя, экспорт отчётов и Swagger-документацию API.
...
- преподаватель создаёт тесты, управляет вопросами и анализирует результаты;
- администратор управляет пользователями, ролями, публикацией тестов и системными настройками;
- комиссия или руководитель может получить осязаемые отчёты в PDF/Excel.
...
- Визуальная аналитика преподавателя:
  - средний балл по тестам;
  - распределение правильных и неправильных ответов по вопросам;
  - время прохождения и результат по попыткам.
- Экспорт аналитики:
  - PDF-отчёт по выбранному тесту;
  - Excel-отчёт по всем тестам преподавателя.
- Админ-панель:
...
В профиле преподавателя и администратора доступен блок `Аналитика преподавателя`.

Он показывает:

- средний балл по тестам;
- количество попыток;
- средний балл по выбранному тесту;
- среднее время прохождения;
- распределение правильных/неправильных ответов по вопросам;
- график времени прохождения и результата по попыткам.

Экспорт:

- `PDF` - отчёт по выбранному тесту;
- `Excel` - сводный отчёт по всем доступным тестам преподавателя, включая листы `Summary`, `Question Stats`, `Attempts`.

Технически используются:
...
При старте backend выполняет синхронизацию моделей. Для поля `durationSeconds` в результатах добавлена безопасная проверка/добавление колонки, чтобы существующая база не ломалась после обновления аналитики.
...
6. Профиль преподавателя с графиками.
7. Экспорт PDF и Excel.
8. Вход под администратором `admin@test.ru / admin123`.
9. Управление ролями пользователей в админке.
10. Swagger UI на `/api/docs`.
11. CI/CD workflow в `.github/workflows/ci-cd.yml`.
